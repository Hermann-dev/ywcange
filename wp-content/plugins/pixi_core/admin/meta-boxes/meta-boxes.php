...
global $custom_post_box ;
 $custom_post_box = array(
	'id' => 'post-meta-box',
	'title' => esc_html__('Post Options','pixi'),
	'page' => 'post',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
		array(
			'name' => esc_html__('Slider Images','pixi'),
			'desc' => esc_html__('If you would like to add more images for a slider, you can add them here.','pixi'),
			'id' => 'mo_image_fields',
			'type' => 'image-blocks',
			'std' => '',
			'options' => array()
...
class motivowebFrameworkMetaboxes {
	...
	function admin_script_loader() {
		...
		if (is_admin() && ($pagenow=='post-new.php' || $pagenow=='post.php')) {
			wp_enqueue_style('tb-metabox', PIXI_ADMIN_URI.'meta-boxes/assets/css/metabox.css');
			wp_enqueue_style('thickbox');
			wp_enqueue_style('colpick', PIXI_ADMIN_URI.'meta-boxes/assets/css/colpick.css'); //colorpicker
			
			wp_enqueue_script('jquery-easytabs', PIXI_ADMIN_URI.'meta-boxes/assets/js/jquery.easytabs.min.js');//page tab
			wp_enqueue_script('blog-tabs', PIXI_ADMIN_URI.'meta-boxes/assets/js/blog.tab.js'); //page tab
			wp_enqueue_script('portfolio', PIXI_ADMIN_URI.'meta-boxes/assets/js/portfolio.js'); //portfolio type
			wp_enqueue_script('meta-box', PIXI_ADMIN_URI.'meta-boxes/assets/js/meta.box.js'); //post type
			wp_enqueue_script('gallery-upload', PIXI_ADMIN_URI.'meta-boxes/assets/js/upload-meta-box.js');//upload gallery
		    wp_enqueue_script('jcolpick', PIXI_ADMIN_URI.'meta-boxes/assets/js/colpick.js'); //colorpicker
	...
	public function add_meta_boxes(){
		...
		$this->add_meta_box('post_options'  , esc_html__('Page Options','pixi')    , 'page');
		$this->add_meta_box('post_options'  , esc_html__('Post Options','pixi')    , 'post');
		$this->add_meta_box('post_options'  , esc_html__('Page Options','pixi')    , 'portfolio');
		$this->add_meta_box('post_options'  , esc_html__('Page Options','pixi')    , 'team');
		
		$this->add_meta_box('post_video'    , esc_html__('Video Settings','pixi')  , 'post');
		$this->add_meta_box('post_audio'    , esc_html__('Audio Settings','pixi')  , 'post');
		$this->add_meta_box('post_quote'    , esc_html__('Quote Settings','pixi')  , 'post');
		$this->add_meta_box('post_link'     , esc_html__('Link Settings','pixi')   , 'post');
		$this->add_meta_box('post_gallery'  , esc_html__('Gallery Settings','pixi'), 'post');
		$this->add_meta_box('custom_gallery', esc_html__('Gallery Settings','pixi'), 'portfolio');
		$this->add_meta_box('post_options'  , esc_html__('Page Options','pixi')    , 'portfolio');
	}
