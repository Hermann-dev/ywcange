<?php
  add_action('widgets_init', 'load_pixi_twitter_widget');
  function load_pixi_twitter_widget()
  {
    register_widget('pixi_Twitter_Widget');
  }
	
	
  class pixi_Twitter_Widget extends WP_Widget {
	 function __construct() {
		parent::__construct(
			'pixi_twitter_widget', // Base ID
			esc_html__( 'Pixi - Twitter Widget', 'pixi'), // Name
			array('classname'=> 'pixi_twitter_widget', 'description' => esc_html__('Display Adv on your site.', 'pixi'),) // Args
        );
	}
	function widget( $args, $instance ) {

		extract( $args );
		$title            = empty( $instance['title'] ) ? esc_html__( 'Latest Tweets', 'pixi' ) : apply_filters( 'widget_title', $instance['title'] );
		...
		if ( ! empty( $twitter_username ) && ! empty( $consumer_key ) && ! empty( $consumer_secret ) ) {
			...
			$token        = get_option( 'pixi_TwitterToken' . $widget_id );
			...
			if ( empty( $twitter_data ) ) {
				...
				if ( ! $token ) {
					...
					if ( $keys ) {
						...
						update_option( 'pixi_TwitterToken' . $widget_id, $keys->access_token );
				...
				foreach ( $twitter_data as $item ) {
					...
					if ( $update ) {
						...
						if ( ( abs( time() - $time ) ) < 86400 ) {
							$h_time = human_time_diff( $time ) . '  ' . esc_html__( 'ago', 'pixi' );
						} else {
	...
	function update( $new_instance, $old_instance ) {
		...
		delete_option( 'pixi_TwitterToken' . $widget_id );
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title'           => esc_html__( 'Latest Tweets', 'pixi' ),
			'no_of_tweets'    => '3',
			'username'        => '@motivoweb1',
			'consumer_key'    => '',
			'consumer_secret' => '',
        ...
        <p>
            <label
                    for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'pixi' ) ?></label>
        ...
        <p>
            <label
                    for="<?php echo esc_attr( $this->get_field_id( 'username' ) ); ?>"><?php esc_html_e( 'Twitter Username:', 'pixi' ) ?></label>
        ...
        <p>
            <label
                    for="<?php echo esc_attr( $this->get_field_id( 'consumer_key' ) ); ?>"><?php esc_html_e( 'Consumer key:', 'pixi' ) ?> </label>
            ...
            <label
                    for="<?php echo esc_attr( $this->get_field_id( 'consumer_secret' ) ); ?>"><?php esc_html_e( 'Consumer secret:', 'pixi' ) ?></label>
        ...
        <p>
            <label
                    for="<?php echo esc_attr( $this->get_field_id( 'no_of_tweets' ) ); ?>"><?php esc_html_e( 'Number of Tweets to show:', 'pixi' ) ?></label>
