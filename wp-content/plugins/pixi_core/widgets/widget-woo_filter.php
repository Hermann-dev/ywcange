<?php
class pixi_Woo_Filter_Attribute extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'pixi' );
		$this->widget_id          = 'woocommerce_layered_nav';
		$this->widget_name        = esc_html__( 'WooCommerce Filter Attribute', 'pixi' );
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by', 'pixi' ),
				'label' => esc_html__( 'Title', 'pixi' )
			),
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => esc_html__( 'Attribute', 'pixi' ),
				'options' => $attribute_array
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => esc_html__( 'Display type', 'pixi' ),
				'options' => array(
					'list'     => esc_html__( 'List', 'pixi' ),
					'dropdown' => esc_html__( 'Dropdown', 'pixi' ),
			...
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => esc_html__( 'Query type', 'pixi' ),
				'options' => array(
					'and' => esc_html__( 'AND', 'pixi' ),
					'or'  => esc_html__( 'OR', 'pixi' )
				)
			),
		);
	}
		...
		if ( 0 < count( $terms ) ) {
			...
			if ( 'dropdown' == $display_type ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( esc_html__( 'Any %s', 'pixi' ), wc_attribute_label( $taxonomy ) ) . '</option>';
...
/**
 * Class pixi_Woo_Filter_Attribute
 */
function register_woo_filter_attribute_widget() {
    register_widget('pixi_Woo_Filter_Attribute');
}
