<?php
function pixi_image_carousel_func($atts, $content = null) {
			...
			<div class="image-carousel" data-col_lg="<?php echo esc_attr($col_lg); ?>" data-col_md="<?php echo esc_attr($col_md); ?>" data-col_sm="<?php echo esc_attr($col_sm); ?>" data-col_xs="<?php echo esc_attr($col_xs); ?>" data-item_space="<?php echo esc_attr($item_space); ?>" data-loop="<?php echo esc_attr($loop); ?>" data-autoplay="<?php echo esc_attr($autoplay); ?>" data-smartspeed="<?php echo esc_attr($smartspeed); ?>" data-nav="<?php echo esc_attr($nav); ?>" data-dots="<?php echo esc_attr($dots); ?>">
				<?php
					foreach($image_ids as $key => $image_id) {
						...
						switch ($click_action) {
							case 'custom_links':
								echo '<a href="'.esc_url($image_link).'"><img src="'.esc_url($attachment[0]).'" alt="'.esc_attr__('image','pixi').'"/></a>';
								break;
							case 'light_box':
								echo '<a class="lightbox" data-imagelightbox-thumbnail="" href="'.esc_url($full_img[0]).'"><img src="'.esc_url($attachment[0]).'" alt="'.esc_attr__('image','pixi').'"/></a>';
								break;
							default:
								echo '<img src="'.esc_url($attachment[0]).'" alt="'.esc_attr__('image','pixi').'"/>';
								break;
						}
					}
				?>
...
if(function_exists('pixi_shortcode')) { pixi_shortcode('image_carousel', 'pixi_image_carousel_func');}