...
function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
    ...
    if ( ! empty( $item->_invalid ) ) {
        ...
        $title = sprintf( esc_html__( '%s (Invalid)', 'pixi'), $item->title );
    } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
        $classes[] = 'pending';
        /* translators: %s: title of menu item in draft status */
        $title = sprintf( esc_html__('%s (Pending)', 'pixi'), $item->title );
    }
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr__('Move up','pixi'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                            echo wp_nonce_url(
                                add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr__('Move down','pixi'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr__('Edit Menu Item','pixi'); ?>" href="<?php
                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                    ?>"><?php esc_html_e( 'Edit Menu Item' ,'pixi'); ?></a>
                </span>
            </dt>
        </dl>
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Navigation Label' ,'pixi'); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Title Attribute','pixi' ); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
                    ...
                    <?php esc_html_e( 'Open link in a new window/tab' ,'pixi'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'CSS Classes (optional)' ,'pixi'); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Link Relationship (XFN)' ,'pixi'); ?><br />
                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                </label>
            </p>
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Description' ,'pixi'); ?><br />
                    ...
                    <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','pixi'); ?></span>
                </label>
            </p>
            <?php
            /*
			...
			if ( ! $depth ) {
				...
				<p class="description description-thin">
					...
					<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
						<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
							<option value="standard" <?php echo esc_attr( $value == 'standard' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Standard Dropdown', 'pixi' ); ?></option>
							<option value="columns2" <?php echo esc_attr( $value == 'columns2' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '2 columns Dropdown', 'pixi' ); ?></option>
							<option value="columns3" <?php echo esc_attr( $value == 'columns3' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '3 columns Dropdown', 'pixi' ); ?></option>
							<option value="columns4" <?php echo esc_attr( $value == 'columns4' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '4 columns Dropdown', 'pixi' ); ?></option>
						</select>
					</label>
				</p>
				<?php
				...
				<p class="description description-thin">
					...
					<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
						<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
							<option value="algleft" <?php echo esc_attr( $value == 'algleft' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Alignleft Dropdown', 'pixi' ); ?></option>
							<option value="algright" <?php echo esc_attr( $value == 'algright' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Alignright Dropdown', 'pixi' ); ?></option>
							<option value="algcenter" <?php echo esc_attr( $value == 'algcenter' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Aligncenter Dropdown', 'pixi' ); ?></option>
							<option value="fullwidth" <?php echo esc_attr( $value == 'fullwidth' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Fullwidth Dropdown', 'pixi' ); ?></option>
						</select>
					</label>
				</p>
				<?php
			...
			<p class="description description-thin"<?php echo esc_attr( $style ); ?>>
				...
				<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
					<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
						<option value="" <?php echo esc_attr( $value == '' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Select Widget Area', 'pixi' ); ?></option>
						<?php
			...
			<p class="description description-thin">
				...
				<label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
					<select id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>">
						<option value="0" <?php echo esc_attr( $value == '0' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'No', 'pixi' ); ?></option>
						<option value="1" <?php echo esc_attr( $value == '1' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Yes', 'pixi' ); ?></option>
					</select>
				</label>
			</p>
		<?php }
        ...
        <p class="description description-thin">
            ...
            <p class="description description-wide">
                <label for="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>">
                    <?php echo esc_html( $title ); ?><br />
                    <input type="text" value="<?php echo esc_attr( $value ); ?>" id="edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class=" <?php echo esc_attr( $key ); ?>" name="<?php echo esc_attr( $key . "[" . $item_id . "]" ); ?>" />
                    <button id="browse-edit-<?php echo esc_attr( $key . '-' . $item_id ); ?>" class="set_custom_images button button-secondary submit-add-to-menu"><?php esc_html_e( 'Browse Image', 'pixi' ); ?></button>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( esc_html__('Original: %s', 'pixi'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ); ?>"><?php esc_html_e('Remove','pixi'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e('Cancel','pixi'); ?></a>
            </div>
...
if(!function_exists('pixi_get_main_menu_parent_items')){
    function pixi_get_main_menu_parent_items(){
        $menu_name = 'mo_main_menu';
        $locations = get_nav_menu_locations();
        $items = array();
        if ( isset( $locations[ $menu_name ] ) && $locations[ $menu_name ] != 0) {
            $menu_id = $locations[ $menu_name ];
            $items = pixi_get_menu_parent_items($menu_id);
            //get the WPML translated items
            $trans_items = pixi_get_translation_items($menu_id);
...
if(!function_exists('pixi_get_menu_parent_items')){
    function pixi_get_menu_parent_items($menu_id){
...
if(!function_exists('pixi_get_translation_items')){
    function pixi_get_translation_items($main_id){
        $items = array();
            ...
            foreach ($languages as $lang) {
                ...
                if(!empty($code)){
                    ...
                    if(!empty($menu_id_str)){
                        ...
                        if($menu_id!=$main_id){
                            $menu_items = pixi_get_menu_parent_items($menu_id);
                            if(!empty($menu_items)){
